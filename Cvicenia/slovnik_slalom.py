...
def oprav_pretekara():
      ...
      if pretekar in slalom.keys():
            ...
            if pom==1:
                  ...
                  slalom[pretekar][0]=cas1
            elif pom==2:
                  cas2=float(input('Zadajte cas 2.kola '))
                  slalom[pretekar][1]=cas2
            slalom[pretekar][2]=slalom[pretekar][0]+slalom[pretekar][1]
      else:
...
def vypis_tabulky(device):
      if len(slalom)==0:
            ...
            print('='*40,file=f)
            for pretekar,casy in sorted(slalom.items(), key=lambda casy: casy[1]):
                body=0
